...
function AdxChart({ config, setConfig, margin }) {
  ...
  useEffect(() => {
    ...
    svg
      ...
      .selectAll("stop")
      .data([
        { offset: "20%", color: "green", opacity: 0.6 },
        { offset: "75%", color: "white", opacity: 0.6 },
        { offset: "100%", color: "red", opacity: 0.6 },
      ])
      .enter()
      .append("stop")
      ...
      .text(
        config.selected
          ? `${d3.timeFormat("%d %b, %Y")(
            ...
            )}: ${d3.format(",.0f")(config.data[config.selected].trend_adx)}%`
          : ""
      );
