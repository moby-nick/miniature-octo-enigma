...
function MovingAverageChart({ config, setConfig, margin }) {
  ...
  useEffect(() => {
    ...
    if(selected) {
      ...
      .text(`${d3.timeFormat("%d %b, %Y")(config.data[selected].date)}: ${d3.format("$,.2f")(config.data[selected].trend_sma_slow)}`);
    }
    ...
    content
      ...
      .attr("cy", config.selected ? y(config.data[selected].trend_sma_slow) : 0)
      .attr("r", 5)
